    ...
    <section className="w-full bg-white">
      ...
      <div className="py-20 bg-white text-gray-800">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <ul className="space-y-4 text-lg md:text-xl">
            {[
              "Are invisible aligners suitable for adults and teens? Yes, the aligners we use at MaxAlign are invisible, affordable, and perfect for both adults and teens to maintain healthy gums and teeth.",
              "How long does the treatment with clear aligners take?",
              "Are invisible aligners painful?",
              "Are clear braces affordable at MaxAlign Dental Clinic?",
              "Can invisible aligners correct all types of dental alignment issues?"
            ].map((item, idx) => (
              <li key={idx} className="flex items-start space-x-3">
                <FaCheckCircle className="text-[#0B7A75] mt-1 flex-shrink-0" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
